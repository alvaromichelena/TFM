...
def visualizacion_planos(bbdd_planos, coef_1, coef_2, indep, SP, a0, a1):
    ...
    ax1.scatter(a0, a1, b0, c = 'k', label = "Punto obtenido", s=4)
    ax1.plot_surface(xx, yy, z, alpha=0.3)
...
def modelo_interpolacion(bbdd_centroides, bbdd_planos, SP):
    ...
    if SP in valores_sp_bbdd:
        ...
        visualizacion_planos(bbdd_planos, coef_1, coef_2, indep, SP, a0, a1)
        """
        #interpolacion_logaritmica
        # Calculo del centroide
        a0 = centroide_menor[0]+((np.log10(centroide_mayor[0]) - np.log10(centroide_menor[0]))/(np.log10(valor_mayor)-np.log10(valor_menor)))*(SP-valor_menor)
        a1 = centroide_menor[1]+((np.log10(centroide_mayor[1]) - np.log10(centroide_menor[1]))/(np.log10(valor_mayor)-np.log10(valor_menor)))*(SP-valor_menor)
        # calculo del plano interpolado
        coef_1 = plano_menor[0]+((np.log10(plano_mayor[0]) - np.log10(plano_menor[0]))/(np.log10(valor_mayor)-np.log10(valor_menor)))*(SP-valor_menor)
        coef_2 = plano_menor[1]+((np.log10(plano_mayor[1]) - np.log10(plano_menor[1]))/(np.log10(valor_mayor)-np.log10(valor_menor)))*(SP-valor_menor)
        indep = plano_menor[2]+((np.log10(plano_mayor[2]) - np.log10(plano_menor[2]))/(np.log10(valor_mayor)-np.log10(valor_menor)))*(SP-valor_menor)
        
        visualizacion_planos(bbdd_planos, coef_1, coef_2, indep, SP)
        """
...
def main():
    ...
    Num_modelo, Den_modelo = modelo_interpolacion(diccionario_centroides, diccionario_planos, SP)

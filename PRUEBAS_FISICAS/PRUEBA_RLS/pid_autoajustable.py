...
def calculo_constantes_ap(numerador, denominador, a=10):
    ...
    El método de autoajuste empleado es el de asignación de polos.
    ...
    Argumentos:
        - numerador: coeficientes del numerador de la función de transferencia (float).
        - denominador: coeficientes del denominador de la función de transferencia (list).
        - a: valor del polo triple en el plano z (mayor que 1).
...
def calculo_constantes_mf(numerador, denominador, Tm=1, k=1):
    ...
    Argumentos:
        ...
        - denominador: coeficientes del denominador de la función de transferencia (list).
        - Tm: tiempo de muestreo (int).
        - k: valor del retardo que se le añade al sistema (uno) (int).
...
def PID_autoajustable(p0, p1, p2, error, u, Km = 1):
    ...
    Argumentos:
        - p0: constante de trabajo del PID.
        - p1: constante de trabajo del PID.
        - p2: constante de trabajo del PID.
        - error: lista de valores de los errores medidos.
        - u: lista de valores de control generados.
        - Km: ganancia en serie con el regulador (por defecto 1 para que no afecte al PID)

    Returns:
        - s_control: señal de control
...
def RLS(entrada_sistema,salida_sistema,n,d,P,X,f_olvido=0.98): 
    ...
    if X.size == 0 or P.size == 0:
        ...
        P = np.identity(3) * 10000 #valor grande inicial
    ...
    K = np.dot(P, X)/(f_olvido + np.dot(X.T, np.dot(P, X)))

    #calculo del error
    
    Error = salida_sistema - np.dot(X.T, Theta)
    ...
    P = (1 / f_olvido) * (P - np.dot(K, np.dot(X.T, P)))
